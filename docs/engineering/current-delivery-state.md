...
| Drafting | On-demand Anthropic drafting produces an Anschreiben and e-mail body from the free-form profile and the selected posting. The prompt asks for the advert's own spelling of every supported term, forbids the stock phrases recruiters read as generated, wants an example with every claim and one of three openings per letter. A sample that still carries a stock phrase, a sentence copied from the advert or an opening reused from an earlier letter is requested once more; the result is accepted either way. |
| Letter quality measures | A deterministic vocabulary of the candidate's market reports which of the advert's terms the letter and the configured CV carry, and which appear in neither; stock phrases, copied eight-word spans and reused openings are reported as notes. Computed from stored text at read time (no column, no model call) and shown per row in the Postausgang. |
