...
| --- | --- |
| Job discovery | Parallel polling of Arbeitsagentur, Jooble, and Arbeitnow through a common adapter contract. Search-call failures are isolated. Every page of an Arbeitsagentur result is read, because the board's default order is not by date, and a page failing after the first loses that page only. Before any detail request the corpus is asked which postings it already holds, whatever their status; only unknown ones are fetched and stored. Arbeitnow keywords match the title and tags, not the body. When the candidate's hard requirements exclude training positions (global or per profile, read by the same function as the scorer's knock-out), the Arbeitsagentur query asks for employment offers only and Arbeitnow items the board classifies as working-student or intern positions, or whose title says so, are withheld; Jooble has no such filter and relies on the knock-out. |
