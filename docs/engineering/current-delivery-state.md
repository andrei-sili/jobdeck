...
- An attachment the pass makes moves the application's last-contact anchor,
  which the silence rule and the company cooling-off window measure from. The
  attachment can be undone from the filed view, but until it is, a mail tied to
  the wrong application delays that application's silence closure.
