...
| Reply tracking | Gmail history polling, deterministic and optional Anthropic classification, matching, review, Gmail labels, and status history. Matching runs thread, then sender address, then form receipt, then sender domain, then company name. The company-name arm compares whole names against an employer's own domain, reads the employer from the tenant slot or the display name when the sender is a job board or an ATS vendor, and only ever proposes; a rescan re-judges its proposals the candidate has not answered. |
...
- Reply matching by company name refuses a mail that fits more than one
  ...
  guessed. A domain that abbreviates the company to fewer than four letters
  is not recognised by name.
